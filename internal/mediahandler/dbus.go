...
func NewDbusMediaHandler() MediaHandler {
...
func (h *DbusMediaHandler) VolumeUp() int {
	...
	return getAudioLevel(h.log)
}

func (h *DbusMediaHandler) VolumeDown() int {
	conn, err := dbus.ConnectSessionBus()
	if err != nil {
		panic(err)
	}
...
func (h *DbusMediaHandler) callMethod(method string) {
	...
	for _, name := range names {
		h.log.Debugf("name : %s, %t", name, strings.Contains(name, "mpris"))
